...
INSTRUCTOR UPDATE (applied):
  • Utility formula uses the corrected form:
      UtilityScore = clinical_outcome × (1 − Severity / 10)
  • Group ID (210) is used as the seed and parameter source throughout.
...
def simulate_treatment(patient_row, medicine_idx, probs):
    ...
    # Corrected utility formula (per instructor update):
    #   UtilityScore = clinical_outcome × (1 − Severity / 10)
    # Reward is 0 if not recovered; decreases with severity if recovered.
